...
async function logout() {
  await axios.get(`${auth_store.api}/logout`)
    .then(() => {
      auth_store.logout()
      router.push({
        name: 'login'
  ...
  <div class="relative" ref="target">
    <router-link class="flex items-center gap-4" to="#" @click.prevent="dropdownOpen = !dropdownOpen">
      <span class="hidden text-right lg:block">
        <span class="block text-sm font-medium text-black dark:text-white"> {{ auth_store.user.name }}</span>
      </span>

      <span class="h-12 w-12 rounded-full">
        <fa-icon icon="user" size="xl" class="py-3 px-2" />
      </span>
